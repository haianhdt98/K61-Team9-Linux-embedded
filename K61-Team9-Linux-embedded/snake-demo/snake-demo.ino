...
int freq,tb;
int l,r,u,d,p;
unsigned long time=150,beeptime=50;
int score=0,flag=0;
...
void setup()
{
  ...
  for(i=0;i<slength;i++)         //Draw the snake
     {
     display.fillRect(x[i],y[i],3,3,BLACK);
     }
     display.display();
...
void checkegg()      //Snake meets egg
{
  if(x[0]==xegg or x[0]==(xegg+1) or x[0]==(xegg+2) or x[0]==(xegg-1))      //Snake in close vicinity of egg
  {
    if(y[0]==yegg or y[0]==(yegg+1) or y[0]==(yegg+2) or y[0]==(yegg-1))
    {
      ...
      xegg=random(1,26)*3+1;              //Create New egg randomly
      yegg=random(1,13)*3+1;
    }
  }
}  
...
void drawsnake()        //Draw snake and egg at newly changed positions
{
  ...
  display.fillRect(x[0],y[0],3,3,BLACK);       //Draw new head of snake
  display.fillRect(x[slength],y[slength],3,3,WHITE);   //Delete old tail of snake
